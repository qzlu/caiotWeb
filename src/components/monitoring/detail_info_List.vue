 ...
 <div class="detail">
	 ...
	 <div class="det_top" v-if="datalist02.DeviceTypeID==801||datalist02.DeviceTypeID==800">
		...
		<div class="video-container" id="innerVideo">
			<video v-if="video_div" id='my-video'  class='video-js vjs-big-play-centered' controls preload='auto' poster='/static/image/index/video_start.jpg'>
	 ...
	 <div v-else>
	 	...
	 	<div class="det_left" >
	 		...
	 		<ul>
	 		  <li>
	 		  	...
	 		  	<div class="a1">
	 		  		<div class="jg"><p class="t" :title="datalist02.DeviceName">{{datalist02.DeviceName}}<!--1#变压器--></p></div>
	 		  		<div v-for="(c,key) in datalist02.DataDetail" :class="setclass(c.SDataID)">
	 		  			
		 		  		<p class="gre" ><i v-for="(df,key) in c.SDataValue" :data="df.DStatus" @click="click_show_line(c.SDataID)">{{df.DValue}}/</i></p>
		 		  		<p class="grd">{{c.SDataTitle}} ({{c.SDataUnit}})<!--三相线电压(V)--></p>
		 		  		<p class="line"></p>
	 		  	...
	 		  	<div class="a2">
	 		  		<div>
	 		  		<p><i class="icon iconfont" :class="datalist02.WebIconName" :data="datalist02.DeviceStatus" style="font-size: 50px;"></i></p>
	 		  		<p class="tx">
	 		  			<i class="icon iconfont"  :data="datalist02.DeviceStatus" >{{datalist02.DeviceStatusTitle}}</i>
	 		  			<!--合闸-->
	 		  		</p>
	 		  		</div>
	 	...
	 	<div class="det_rightItem">
	 		<div class="det_titles" style="width: 110px; height: 165px; padding: 25px 0 0 20px; float: left; font-size: 18px; color: #F1F2F2;">实时监测</div>
	 		<div class="show_acr_c" style="width: 900px; height: 185px; float: left; color: #fff; margin-top: 19px;" id="container_vn"></div>	 		
	 	</div>
 ...
 export default {
      data() {
        return {
			...
			linedata:[],//图表线条数据
			video_div:false,
			videoUrl:'http://hls.open.ys7.com/openlive/669cf1ba63a34653a1c358b17ceea2b6.m3u8'
        }
      },
	created(){
	...
	getInfo(info){
		if(this.datalist02.DeviceTypeID==801||this.datalist02.DeviceTypeID==800){
	...
	line_data_reset(){ //重新组装线条图形表
	
		 //console.log(this.linedata)
		let d=this.linedata;
		let arr_name=[],x_arr=[],y_arr=[];
		
		for(let a of d){
			arr_name.push(a.DataItemName)//把名字写入数组
		}
		...
		/*组装成line图表相应的格式数据*/
		for(let u of d){
			let obj={type:"line",stack:'总量'},y_datas=[];
			    obj.name=u.DataItemName
			    for(let m of u.ProjectChartLineDatas){
			    	y_datas.push(m.Y)
			    }
			   //console.log(y_datas)
			   obj.data=y_datas;
	...
	line_chart(name,x,ydatas){//生成line 坐标图
		//console.log(x)
		    ...
		    /*series: [
		        {
		            ...
		            type:'line',
		            stack: '总量',
		            data:[2, 22, 10, 14, 19, 23, 10, 20, 25]
		        },
		        {
		            name:'联盟广告',
		            type:'line',
		            stack: '总量',
		            data:[22, 12, 18, 10, 19, 13, 18, 10, 15]
		        },
		        {
		            name:'视频广告',
		            type:'line',
		            stack: '总量',
		            data:[8, 12, 20, 24, 12, 13, 20, 25, 15]
		        },
		        {
		            name:'直接访问',
		            type:'line',
		            stack: '总量',
		            data:[18, 12, 14, 24, 15, 13, 18, 25, 15]
		        },
		        {
		            name:'搜索引擎',
		            type:'line',
		            stack: '总量',
		            data:[7, 12, 20, 24, 14, 23, 16, 10, 25]
		        }
		    ],*/
    ...
    mounted:function(){  	
		      ...
		      function settimeouts_detil_info_list(){
					...
					let timeoutId_detal_info_list =setTimeout(settimeouts_detil_info_list, 24000);
...
.det_top{ height: 227px; position: relative; }

 .det_top .det_left{  position: relative;    margin-top: -6px;width: 456px; height: 230px; float: left; background: url(/static/image/indexdetail/bg_blue.png) no-repeat; }
...
.det_top .det_left .dmes img{ vertical-align: -2px;}
...
.det_top .det_left .dt_kgs>div{ padding: 0 0 70px 0;}
 ...
 .det_top .det_left .dt_kgs>div p.n{ color: #03CD82;font-size: 25px;}
   ...
   .det_top .det_left .ge_pelin{ position: absolute;top: 117px; left: 170px;}
  
 .det_top .det_left .de_arc_li{ width: 310px; height: 148px; margin-top: -8px; float: left;background: url(/static/image/index/leftbar_bg_circle.png) no-repeat;}
  .det_top .det_left .de_numli{ width: 115px; height: 120px;   position: absolute; top: 97px; right: 10px;}
 ...
 .det_top .det_rightItem{ width: 1086px; height: 227px; margin:-5px 15px 0 0;background: url(/static/image/indexdetail/bg_blue_2.png) no-repeat; float: right;}
  ...
  .detail .det_itemList{ width: 1559px;  height: 690px; background: url(/static/image/indexdetail/content_fg_1.png);}
.detail .det_itemList .mains{ padding: 0 5px 0 20px;margin-right: 5px; height: 600px; overflow:hidden;}
.detail .det_tlg{ /*padding: 25px 16px 0 20px;*/ overflow: hidden;}
.detail  .pjngh{padding: 2px 16px 0 20px;}



 .det_left ul li{  padding: 15px 25px 5px 25px; height: 194px;  }
 .det_left ul li.err .inner_bg_sh{ width:100%;  height: 194px ;background: url(/static/image/indexdetail/content_bg_2.png) 0px 0 no-repeat;}
 .det_left ul li .a1{ position: relative; width: 148px; height: 175px; float: left;  padding: 0px 0 0 10px;text-align: left;}

 
 .det_left ul li .a1 .pui_1{ position: absolute; top: 41px; left: 0; width: 140px;}
 .det_left ul li .a1 .pui_2{ position: absolute; top: 88px; left: 0;width: 140px;}
 .det_left ul li .a1 .pui_3{ position: absolute; top: 135px; left: 0;width: 140px;}
 .det_left ul li .a1 .pui_4{ position: absolute; top: 41px; right: -265px;width: 140px;}
 .det_left ul li .a1 .pui_5{ position: absolute; top: 88px; right: -265px;width: 140px;}
 .det_left ul li .a1 .pui_6{ position: absolute; top: 135px; right: -265px;width: 140px;}
 .det_left ul li .a1 .pui_7{ position: absolute; top: 135px; right: -144px;width: 140px; }
 ...
 .det_left ul li .a1 .jg{position:relative; height: 35px;}
 .det_left ul li .a1 .jg .t{ position: absolute; top: 0px; left: -12px; height: 35px; width: 205px;  font-size: 20px; color: #525E7E;white-space:nowrap;}
 .det_left ul li .a1 .gre{ color: #03CD82; font-size: 17px;padding: 1px 0; }
 .det_left ul li .a1 .grd{  font-size: 13px; color: #737373;}
 .det_left ul li .a1 .line{ height: 5px;}

 .det_left ul li .a2{ width: 96px; height: 175px; float: left;display: flex;align-items:center;justify-content:center}
 .det_left ul li .a2 p{ width: 100%;}
 .det_left ul li .a2 p.tx{ font-size: 16px; color: #03CD82;}
 .det_left ul li.err .a2 p.tx{ font-size: 16px; color: #D62445;}
 .det_left ul li .a2 p.p1{ color: #525E7E;}
 .det_left ul li .a2 p.p2{ color: #D62445;}
 .det_left ul li  .lrn{ width: 116px; margin:15px 15px 0 0}

 .det_left ul li .lrn .gre{ color: #03CD82; font-size: 17px;padding: 7px 0; }
 .det_left ul li .lrn .grd{  font-size: 13px; color: #737373;}
 .det_left ul li .lrn .line{ height: 18px;}
  .video-container{
	  width:1562px;
	  height: 546px;
