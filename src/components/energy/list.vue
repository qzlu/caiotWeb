...
<template>
  		 ...
  		 <div class="en_left">
  		 	 ...
  		 	 <section class="ech_time">
  		 	  ...
  		 	  <section class="e_bigdate">
  		 	  	<div class="it01" style="width: 205px;"><p class="t"><!--今日-->{{titles[0]}}</p><p class="nt" style="font-size: 20px;">{{table_data01.CurEnergy}}<!--20357--></p></div>
  		 	  	<div class="it01 nobg" style="width: 205px;">
  		 	  		<p class="t"><!--昨日-->{{titles[1]}}</p>
  		 	  		<p class="np">{{table_data01.PriorEnergy}}</p>
  		 	  		<p class="gc" :class="table_data01.HisPercent>0 ? '' : 'gy'" style="margin-left: 80px;">{{table_data01.HisPercent}}<!--15%--></p>
  		 	  	</div>
  		 	  	<div class="it01 nobg" style="display: none;">
  		 	  		<p class="t">历史平均</p>
  		 	  		<p class="np">{{table_data01.HisAvgEnergy}}</p>
  		 	  		<p class="gc" :class="table_data01.PriorPercent>0 ? '' : 'gy'">{{table_data01.PriorPercent}}<!--15-->%</p>
  		 	  	</div>
  		 	  </section>
 ...
 <style>
.en_container	.el-input__suffix{right: -25px; display: none;}
.en_container .el-date-editor.el-input, .el-date-editor.el-input__inner{ width: 100%; font-size:12px ;  }
.en_container  .el-input--suffix .el-input__inner{padding-right:0 ;}
.en_container .el-input__inner{ height: 20px; text-align: center; background:none; border: none; color: #fff;}
.en_container .el-input__icon{line-height: 20px;}
 </style>
<script>
	var week_start=0;
	     ...
	     //jsons.data.FObject数据：1.Table：总能耗统计数据 ，2Table1：能耗曲线数据，3.Table2：分项能耗数据，4.Table3：重点能耗数据（根据FType为天、周、月、年返回对应值）
                  ...
                  _this.table_data02_name=[];
			               ...
			               data:[120, 132, 101, 134, 90, 230, 210, 310, 210]
								 ...
								 _this.$refs.editchi.getMessage(_this.table_data03,_this.table_data02_name)//调用子组件的方法
								  //this.table_data04,this.table_data02_name 子组件2
								 _this.$refs.editchi02.getMessage02(_this.table_data04,_this.table_data02_name)//调用子组件的方法
...
.en_container .e_header .en_left .ech_time .e_tg .e_set_time{ margin: 6px 0 0 20px; width: 150px; height: 20px; float: left;}  
.en_container .e_header .en_left .ech_time .e_tg .en_tpn { cursor: pointer; width: 128px; height: 20px; float: left; font-size: 14px; color: #F1F1F2; margin: -2px 5px 0 0;}  
...
.en_container .e_header .e_bigdate .it01 .gc{background: url(/static/image/energy/ing_up.png) 3px 5px no-repeat #A21D32; /*background: #A21D32;*/ margin:10px 0 0 50px; width: 52px; height: 20px ; line-height: 20px;border-radius :5px;}
.en_container .e_header .e_bigdate .it01 .gy{background: url(/static/image/energy/ing_down.png) 3px 5px no-repeat #06A167; }
